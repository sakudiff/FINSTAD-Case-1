...
Figure~\ref{fig:viz_viz2}: Closing Price Trends by Asset plots nominal closing prices rather than normalized returns, revealing that BDO experienced meaningful price swings on its own scale despite appearing flat in Figure~\ref{fig:viz_viz1}: Cumulative Returns. Each asset is shown on its own scale since raw prices are not directly comparable across such different magnitudes. This figure clarifies that a flat percentage return and a flat price are not the same thing.
...
Figure~\ref{fig:viz_viz4}: Return Range \& Spread by Asset shows each asset's typical daily return range and outlier days. BTC has the widest box and most extreme outliers in both directions, consistent with its high kurtosis in Figure~\ref{fig:viz_viz3}: Return Distribution with Skewness \& Kurtosis. SPY has the narrowest, most compressed box, reflecting the volatility-dampening effect of diversification across 500 companies. AAPL and BDO fall in between, with BDO showing a visible skew toward downside outlier days.
