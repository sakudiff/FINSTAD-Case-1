...
Table~\ref{tab:q1} establishes the hierarchy of risk-adjusted returns, and the dispersion between top and bottom of the ranking carries immediate portfolio allocation consequences. AAPL leads at 0.0538, a Sharpe-like ratio built on a 0.11\% mean daily return against 2.00\% volatility. BTC follows at 0.0520, achieving near-parity through a fundamentally different mechanism. Where AAPL's efficiency comes from variance containment, BTC's comes from return magnitude. Its 0.17\% mean daily return is the highest in the sample, and it must be, because its 3.19\% standard deviation is the highest too. SPY posts 0.0486 on 1.31\% volatility, the lowest variance in the sample. BDO ranks last at 0.0131, carrying a 2.26\% standard deviation while producing a mean daily return of 0.03\%, roughly one-quarter of the next-lowest asset. The 4x spread between the top and bottom of the ranking is not a measurement artifact.
...
Table~\ref{tab:q2} quantifies the COVID-19 drawdowns. AAPL lost 17.6\% over 41 trading days. SPY lost 19.4\% over the same window. BDO lost 30.1\% across 40 trading days. BTC lost 31.1\% across 60 trading days. The ranking of drawdown severity aligns perfectly with the volatility hierarchy established in Q1, confirming that pre-crisis volatility is the dominant predictor of crisis-period losses. AAPL and SPY, the two lowest-volatility assets in Q1, also recorded the smallest drawdowns. BTC and BDO, the two highest-volatility assets, recorded the largest.

The timing structure of the losses matters as much as the terminal values. SPY and AAPL declined in near lockstep, reflecting the broad-based nature of the selloff across US equities. BTC exhibited sharp intra-period reversals, with violent single-day bounces followed by deeper declines. The worst single-day loss of this period, BTC's 37.2\% decline on 12 March 2020, will be analyzed in detail in Q5 as the defining tail event of the sample. BDO showed a delayed but sustained decline that continued after US equities had begun to stabilize. This lag is the critical signal for an emerging market allocator. A portfolio hedged with US equity put options would have seen the hedge expire before BDO's drawdown completed. The duration of the hedge, not just the strike, must account for the slower transmission of global liquidity shocks to emerging market equities.

The proposition that cryptocurrency serves as a crisis hedge is not merely unproven. It is rejected. BTC recorded the largest cumulative decline in the sample at 31.1\%, exceeding every equity asset. The proposition that continuous trading provides resilience during a crisis is also rejected. BTC's 60 trading days versus 41 for equities mean the losses accumulated over more calendar days. The same continuous trading structure that will be shown in Q5 to enable extreme tail events also extended the duration over which capital could be destroyed. For a risk manager calibrating VaR models to this period, the maximum single-day loss of 37.2\% is not the only relevant input. The fact that the cumulative loss reached 31.1\% despite the single-day loss being the more extreme number implies that the losses were concentrated, not distributed, and any model calibrated to normal-period volatility would have been violated by a margin of 10 standard deviations or more.
...
Table~\ref{tab:q4} ranks assets by total return, and the dispersion across the four assets is severe enough to determine a multi-year performance attribution in a single table. BTC delivered 1,152.7\%, an annualized return of 52.4\%. AAPL returned 278.8\%, annualized at 24.9\%. SPY returned 130.8\%, annualized at 15.0\%. BDO returned 5.0\%, annualized at 0.8\%. The spread from top to bottom exceeds 1,100 percentage points. The question for a portfolio manager is not which asset performed best. The question is how much of this dispersion is explainable by factor exposure and how much is asset-specific.
...
Table~\ref{tab:q8} categorizes returns conditional on the VIX regime, and the asymmetry across regimes is the most actionable portfolio signal in this study. During high-fear periods with a VIX above 30, BTC is the only asset with a positive mean daily return at 0.16\%. AAPL posts -0.54\%. BDO posts -0.42\%. SPY posts -0.44\%. The traditional equity assets exhibit uniform negative returns, consistent with the repricing of risk premiums during volatility spikes. BTC stands apart, and the mechanism connects directly to the correlation architecture from Q3. BTC's moderate unconditional correlation with SPY at 0.383 masks a regime-dependent structure where the correlation declines during high-VIX periods, allowing BTC to deliver positive returns while equities decline.

For a risk-parity or volatility-targeting strategy, this regime dependence presents a tactical allocation opportunity. A quantitative framework that conditions portfolio weights on VIX regime would reduce equity exposure when the VIX exceeds 30 and increase BTC exposure under the same condition. The conditional return tables provide the explicit inputs. The data also confirm that the cross-asset ranking in calm markets is the inverse of the ranking during turmoil. AAPL leads low-fear regimes at 0.25\% per day, doubling BDO at 0.06\%. A portfolio that is long AAPL and short BTC in low-VIX environments and reverses the pair in high-VIX environments would capture a regime-switching premium directly observable in the table. The crisis drawdowns from Q2 represent a specific realization of this mechanism. During the COVID-19 crash, the VIX spiked above 30, BTC's conditional return was positive, and equities declined as predicted by the regime framework.

The limitation on this strategy is sample size. The high-fear regime contains 148 observations per asset, approximately 9.4\% of the total sample. Within those 148 observations, BTC's positive mean return is driven by a specific sequence of events during the COVID-19 crash and the 2022 rate hiking cycle. Whether the relationship generalizes beyond this sample is not knowable from the available data. The correct risk management framework is to position for the regime-dependent return asymmetry but to size the position so that a failure of the relationship to persist does not produce a catastrophic loss. A tail hedge expressing this view through options rather than spot positions would capture the convexity of the relationship without the unlimited downside of a spot long position. The Q5 tail risk estimates provide the upper bound for how much that option protection would cost.
...
Table~\ref{tab:q9} provides the conditional mean returns by yield direction. During rising-yield periods, all four assets deliver positive returns. BTC leads at 0.20\%, followed by SPY at 0.14\%, AAPL at 0.13\%, and BDO at 0.07\%. The uniform positivity contradicts the textbook DCF intuition that higher discount rates mechanically lower asset prices. The resolution is that rising yields during this sample period were predominantly associated with improving growth expectations rather than restrictive monetary policy. The earnings channel dominated the discount rate channel. For a macro-driven portfolio, the correct interpretation is that a long equity position is a long growth expectations position, not a long falling-rate position.

The falling-yield regime exposes the cross-asset divergence. SPY records a mean daily return of -0.01\%, the only negative conditional return in the table. When yields fall because growth expectations are revised downward, equity markets decline because the earnings outlook deteriorates, and the discount rate relief is insufficient to offset the earnings compression. BDO shows a mean return of 0.00\% in falling-yield periods against 0.07\% in rising-yield periods, a difference that is economically negligible. Philippine banking stocks are structurally insulated from US Treasury movements, and the connection to the correlation analysis in Q3 is direct. The same factors that produce BDO's near-zero cross-correlation also produce its near-zero interest rate sensitivity.
