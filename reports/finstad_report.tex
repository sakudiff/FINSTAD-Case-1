...
Refer to Appendix~\ref{sec:appendix_a} (section~Q4) for the R implementation. The analysis was guided by the following inquiry:
...
Bitcoin (BTC) generated the highest absolute return over the six-year period at 1,153\%, reflecting its high-beta profile. Apple (AAPL) and the S\&P 500 (SPY) delivered equity returns of 279\% and 131\%, respectively. BDO Unibank (BDO) exhibited a total return of 5.0\%, consistent with the structural constraints of emerging market banking sectors operating under shifting domestic interest rate regimes.

\subsection{Q5. Best and Worst Trading Days}

Refer to Appendix~\ref{sec:appendix_a} (section~Q5) for the R implementation. The analysis was guided by the following inquiry:
...
BTC recorded the widest spread between its best and worst trading days (+18.7\% to -37.2\%), highlighting the severe tail risks inherent in cryptocurrency markets. SPY exhibited the narrowest range (+10.5\% to -10.9\%), functioning as a diversified macroeconomic anchor. The extreme negative days for equities align with the March 2020 COVID-19 selloff, where panic-induced deleveraging drove historic single-day drawdowns across traditional markets.

\subsection{Q6. Moving Average Crossover Analysis}

Refer to Appendix~\ref{sec:appendix_a} (section~Q6) for the R implementation. The analysis was guided by the following inquiry:
...
Traditional equities display extended periods of directional momentum; AAPL and BDO recorded only 6 crossovers during the 2024--2025 period, while SPY recorded 8. BTC's moving averages intersected 15 times over the same period, confirming that its price discovery is dominated by frequent sentiment shocks rather than gradual institutional accumulation.

\subsection{Q7. Volume and Volatility Relationship}

Refer to Appendix~\ref{sec:appendix_a} (section~Q7) for the R implementation. The analysis was guided by the following inquiry:
...
Mean absolute returns were systematically higher on high-volume days across all asset classes. BTC exhibited a mean absolute return of 3.05\% on high-volume days compared to 1.83\% on normal days. SPY showed a similar proportional expansion, increasing from 0.61\% to 1.60\%. This confirms that liquidity events in these markets are predominantly driven by information shocks that induce aggressive price repricing, rather than benign block trading.
