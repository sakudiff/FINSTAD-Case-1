...
# Escape underscores for LaTeX (interpreted as math subscript otherwise)
esc_underscore <- function(x) gsub("_", "\\\\_", x)
...
cat("\\toprule\n")
cat(paste(esc_underscore(names(desc)), collapse = " & "), "\\\\\n")
cat("\\midrule\n")
for (i in seq_len(nrow(desc))) {
  vals <- desc[i, ]
  formatted <- vapply(seq_along(vals), function(j) {
    if (j == 1L) return(esc_underscore(as.character(vals[[j]])))
...
cat("\\toprule\n")
cat(" & ", paste(esc_underscore(colnames(cor_mat)), collapse = " & "), "\\\\\n")
cat("\\midrule\n")
for (i in seq_len(nrow(cor_mat))) {
  cat(esc_underscore(rownames(cor_mat)[i]))
